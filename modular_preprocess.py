...
def convert_to_img(file,number):
	...
	x=np.array(x)
	cv2.imwrite("visualize/X_train"+str(number)+".png",x) 
	#x=x/x.max()
	#x=x/255

	#print(x.shape)
	#cv2.imwrite("train/X_train"+str(number)+".png",x)
	#return(number)
a=[]
with open("X_train_hash.txt")as ff:
...
for ii in range(0,len(a)):
	...
	#os.system((("wget https://storage.googleapis.com/uga-dsp/project2/data/bytes/"+str(a[ii].strip())+".bytes -O txt_X_train/"+str(counter)+".txt")))
	convert_to_img("txt_X_train/"+str(counter)+".txt",counter)
    #print(hi)
	counter=counter+1
